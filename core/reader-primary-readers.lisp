...
(defun <register-primary-readers-for-mspace> ()
  ...
  ;;    (let (result
  ;;          (cur-pkg *package*))
  ;;      (let* ((*package* (lisp3dev.base:memoized (find-package :lisp3dev.mspace)))
  ;;             (*readtable* lisp3dev.LE.CORE:*QI-READTABLE*)
  ;;             (src (read stream t nil t)))
  ;;        (setf result (if (lisp3dev.LE.CORE::%need-trans-p% src)
  ;;                       (let ((*package* cur-pkg))
  ;;                         (lisp3dev.LE.CORE::%trans% src))
  ;;                       src)))
  ;;      result)))
  ...
  (set-dispatch-macro-character
   ...
   (lambda (stream char1 char2)
     ...
     (let ((*package* (memoized (find-package :lisp3dev.mspace)))
