...
(in-package :lisp3dev.base.match)
...
(defmacro LISP3DEV.BASE.UNIFY:u-case (x &body clauses)
  (with-gensyms (tmp)
    `(let ((,tmp ,x))
      ,(<*-case> 'u-case 'if 'do-unify-if tmp nil clauses))))
(defmacro LISP3DEV.BASE.MATCH:m-case (x &body clauses)
  (with-gensyms (tmp)
    `(let ((,tmp ,x))
      ,(<*-case> 'm-case 'if 'do-match-if tmp nil clauses))))

(defmacro LISP3DEV.BASE.UNIFY:u-gcase (x &body clauses)
  (with-gensyms (tmp)
    `(let ((,tmp ,x))
      ,(<*-case> 'u-gcase 'gif 'do-unify-gif tmp nil clauses))))
(defmacro LISP3DEV.BASE.MATCH:m-gcase (x &body clauses)
  (with-gensyms (tmp)
    `(let ((,tmp ,x))
...
(defmacro LISP3DEV.BASE.UNIFY:u-case/w ((x &rest options) &body clauses)
  (<*-case/w> 'u-case/w 'if 'do-unify-if x options clauses))
(defmacro LISP3DEV.BASE.MATCH:m-case/w ((x &rest options) &body clauses)
  (<*-case/w> 'm-case/w 'if 'do-match-if x options clauses))

(defmacro LISP3DEV.BASE.UNIFY:u-gcase/w ((x &rest options) &body clauses)
  (<*-case/w> 'u-case/w 'gif 'do-unify-gif x options clauses))
(defmacro LISP3DEV.BASE.MATCH:m-gcase/w ((x &rest options) &body clauses)
  (<*-case/w> 'm-case/w 'gif 'do-match-gif x options clauses))


(defmacro LISP3DEV.BASE.UNIFY:u-acase (x &body clauses)
  `(let ((it ,x))
    (u-case it ,@clauses)))
(defmacro LISP3DEV.BASE.MATCH:m-acase (x &body clauses)
  `(let ((it ,x))
    (m-case it ,@clauses)))

(defmacro LISP3DEV.BASE.UNIFY:u-gacase (x &body clauses)
  `(let ((it ,x))
    (u-gcase it ,@clauses)))
(defmacro LISP3DEV.BASE.MATCH:m-gacase (x &body clauses)
  `(let ((it ,x))
    (m-gcase it ,@clauses)))
...
(defmacro LISP3DEV.BASE.UNIFY:u-acase/w ((x &rest options) &body clauses)
  (<*-acase/w> 'u-acase/w 'u-case/w x options clauses))
(defmacro LISP3DEV.BASE.MATCH:m-acase/w ((x &rest options) &body clauses)
  (<*-acase/w> 'm-acase/w 'm-case/w x options clauses))

(defmacro LISP3DEV.BASE.UNIFY:u-gacase/w ((x &rest options) &body clauses)
  (<*-acase/w> 'u-gacase/w 'u-gcase/w x options clauses))
(defmacro LISP3DEV.BASE.MATCH:m-gacase/w ((x &rest options) &body clauses)
...
(defmacro LISP3DEV.BASE.UNIFY:u-cond (&body clauses)
  (<*-cond> 'u-cond 'if 'do-unify-if clauses))
(defmacro LISP3DEV.BASE.MATCH:m-cond (&body clauses)
  (<*-cond> 'm-cond 'if 'do-match-if clauses))

(defmacro LISP3DEV.BASE.UNIFY:u-gcond (&body clauses)
  (<*-cond> 'u-gcond 'gif 'do-unify-gif clauses))
(defmacro LISP3DEV.BASE.MATCH:m-gcond (&body clauses)
...
(defmacro LISP3DEV.BASE.UNIFY:u-bind ((&rest pairs) &body body)
  (<*-bind> 'u-bind 'do-unify-if pairs body))
(defmacro LISP3DEV.BASE.MATCH:m-bind ((&rest pairs) &body body)
