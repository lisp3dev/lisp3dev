...
(defpackage :lisp3dev.base.unify-global-var (:use))

(lisp3dev.core:define-package :lisp3dev.base.unify* (:lisp3dev.base.unify)
  (:use :cl)
  (:import/export :lisp3dev.base.hash-table*)
  (:export 
...
;@eval-always
(defvar *pkg-unify-global-var* (find-package :lisp3dev.base.unify-global-var))
...
;; 普通は構造化の意図を持って使用される（リスト・ベクタに対する評価順序の制御）
 ...
 #|
 @select-reader lisp3dev.core.reader::%internal%
 ...
 (set-macro-character 
  ...
  (lambda (stream char)
    ...
    (let* ((_ (error "EXPERIMENTAL TEST ERROR"))
           ...
           (main (member-if-not #/(and (symbolp _) (lisp3dev.base.unify::is-var? _))
                                contents))
...
(defmacro internal-label (x) 
  `(intern (concatenate 'string "_unif_label_" (symbol-name ,x)) :lisp3dev.base.unify))
...
(defun special-post-code (ptn dst var info &aux (h (car ptn)))
  ...
  (acond ((or (eq :member h)
        ...
        ((member h '(:length= :length/= :length< :length<= :length> :length>=))
         `(and 
           (typep ,var 'sequence)
           (,(intern (symbol-name h) :lisp3dev.base.unify) ,(second ptn) ,var)))
           
        ((member h '(:length<< :length<=< :length<=<= :length<<=))
         `(and 
           (typep ,var 'sequence)
           (,(intern (symbol-name h) :lisp3dev.base.unify) ,(second ptn) ,(third ptn) ,var)))
         ...
         (if is-a-cons?
           ...
           ((eq :list* head)
             (unless (and (proper-list-p (cdr ptn))
                          (<= 2 (length (cdr ptn))))
               (error "LIST*"))
             (let* ((x (intern "!lisp3dev.base.unify-list*-tmpvar!"))
                    (x? (memoized (intern "??!lisp3dev.base.unify-list*-tmpvar!"
