...
(lisp3dev.core:define-package :lisp3dev.base.hlist* (:lisp3dev.base.hlist)
  (:use :cl)
  (:import/export :lisp3dev.base.list*)
  (:export
...
(in-package :lisp3dev.base.hlist)
...
(defun bool-rand () (zerop (random 2)))
 ...
 (let* ((s (read-file-into-string "u:/files"))
        ...
        tmp)
   (dolist (x lines)
     (let* ((vec (lisp3dev.base.text::text-scan  "(.*?)-(.*)" x :output :sub-strings-as-list))
            (author (first vec))
            (title (second vec)))
...
(lisp3dev.base.text::text-scan  "(.*)-(.*)" "a-aa" :output :sub-strings)
